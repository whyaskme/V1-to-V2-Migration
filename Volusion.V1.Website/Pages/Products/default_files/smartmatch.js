...
function TCN_addContent(str) {
    //duplicate spaces were breaking the drop down so I removed them here
    //I am not sure if this is the best option but the fix for other characters did not work with spaces
    if (str.indexOf(" ") >= 0) {
        str = str.replace(/\s+/g, " ");
    }
	...
	if(index!=-1){
		...
		if(index>0){
		    for (var p = 0; p < index; p++) {
		        thisComboStr += comboGroup[p].options[comboGroup[p].selectedIndex].optionText + separator + comboGroup[p].options[comboGroup[p].selectedIndex].value + separator;
			}
		}
		...
		for(var i=0;i<contents.length;i++){
			...
			if (contents[i].substr(0, thisComboStr.length) == thisComboStr) {
			    for (var j = 0; j < thisCombo.options.length; j++) {
			        ...
			        if (arrayValues[i][thisComboValue] == thisCombo.options[j].value) {
			            existe = true;
			        }
			    }
			    if (existe == false) {
			        ...
			        thisCombo.options[newIndex].optionText = arrayValues[i][thisComboText];
